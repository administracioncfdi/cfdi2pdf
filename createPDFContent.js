...
var generateStampTable = function(timbreFiscalDigital, re, rr, tt){
  if(timbreFiscalDigital){
    return {
        table: {
            widths: [150,100,237],
            body: [
                  [{colSpan: 1, rowSpan: 4, qr: generateQrCode(timbreFiscalDigital, re, rr, tt) , fit:150},'NUMERO SERIE CERTIFICADO',checkIfExists(timbreFiscalDigital.noCertificadoSAT)],
                  ['', 'FECHA HORA CERTIFICACION', timbreFiscalDigital.fechaTimbrado ? timbreFiscalDigital.fechaTimbrado.substring(0,10) + timbreFiscalDigital.fechaTimbrado.substring(11,19) : ""],
                  ['', 'FOLIO FISCAL UUID', checkIfExists(timbreFiscalDigital.uuid)],
                  ['', 'SELLO DIGITAL', checkIfExists(timbreFiscalDigital.selloCFD)],
                  ['SELLO DEL SAT',{colSpan: 2, text: checkIfExists(timbreFiscalDigital.selloSAT)}],
  ...
  var dd = {
  		...
  		'\n',
      {
        ...
        layout: {
  		...
  		'\n',
      generateStampTable(json.timbreFiscalDigital, json.emisor.rfc, json.receptor.rfc, json.total)
  	],
  	styles: {
  		header: {
  			fontSize: 18,
